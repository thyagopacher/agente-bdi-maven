...
public class NullObjectPlan extends DesignPattern implements java.io.Serializable {
		...
		if (m.getParameters().size() >= 1 && !m.isPrivate()) {
			...
			for (Object s : variaveisDeclaradas) {
				if (s instanceof ExpressionStmt) {
					...
					if(variavelDeclarada.getExpression().getChildNodes().isEmpty()
							|| variavelDeclarada.getExpression().getChildNodes().size() < 2) {
						return false;
					}
	...
	public boolean isOptional(FieldDeclaration f, String nomeClasse) {
		...
		int qtdConstrutor = object.getType().getMembers().stream().filter(l -> l instanceof ConstructorDeclaration)
				.collect(Collectors.toList()).size();
		if (qtdConstrutor > 0) {
			ConstructorDeclaration construtor = (ConstructorDeclaration) object.getType().getMembers().stream()
					.filter(l -> l instanceof ConstructorDeclaration).collect(Collectors.toList()).get(0);
	...
	public boolean violatesOptionalFieldPreconditions(FieldDeclaration f) {
		try {
			...
			if (arquivo.exists()) {
				System.out.println("Lendo arquivo em método null object: " + arquivo);
			...
			for (FieldDeclaration campo : camposClasse) {
				...
				if (!metodos.isEmpty()) {
					for (MethodDeclaration metodo : metodos) {
						if (metodo.isConstructorDeclaration() && metodo.isPrivate()) {
							continue;
						}
							...
							if (linhasMetodo != null && !linhasMetodo.isEmpty()) {
								for (Statement linha : linhasMetodo) {
									if (linha instanceof IfStmt) {
										...
										boolean campoTemNoCondicional = condicional.getCondition().toString()
												.contains(campo.getVariable(0).toString());
										System.out.println("Res: " + campoTemNoCondicional + " Nome: "
												+ campo.getVariable(0).toString());
										if (!campoTemNoCondicional) {
											continue;
										}
	...
	public void applyMethod(ProprietyClass object) {
		System.out.println("Classe aplicando nullobject:" + object.getAbsolutePath());
		String nomePacote = "";
		if(object.getCu().getPackageDeclaration() != null && object.getCu().getPackageDeclaration().isPresent()) {
			nomePacote = object.getCu().getPackageDeclaration().get().getNameAsString();
		}
		...
		if (objectNull == null || objectNull.getType() == null) {
			System.out.println("Não achou classe NULL: " + nomeObjeto);
		} else {
			...
			if (!arquivoAbstratoExiste || !arquivoNullExiste) {
				...
				if (!arquivoNullExiste) {
					...
					if (!type2.getExtendedTypes().isEmpty() && !type2.getExtendedTypes(0).toString().contains(nomeClasse)) {
				...
				for (BodyDeclaration<?> member : metodosObjeto) {
					if (member instanceof MethodDeclaration) {
						...
						if (!arquivoAbstratoExiste) {
							MethodDeclaration methodAbstrato = new MethodDeclaration(modificadorMetodo.clone(),
									method0.getType(), method0.getNameAsString());
						...
						if (!arquivoNullExiste) {
							MethodDeclaration methodNull = new MethodDeclaration(modificadorMetodo, method0.getType(),
									method0.getNameAsString());
				...
				if (!arquivoAbstratoExiste) {
					this.addMethod("isNull", "boolean", type1, "", EnumSet.of(Modifier.PUBLIC, Modifier.ABSTRACT),
							null);
					this.addMethod("getReference", nomeObjeto, type1, "",
							EnumSet.of(Modifier.PUBLIC, Modifier.ABSTRACT), null);
				 ...
				 * abstrata
				 */
				saveContent(object.getParentFile().toString() + "/" + nomeObjeto + ".java", object.getCu().toString());
					...
					if (linha instanceof IfStmt) {
						if (linha.toString().contains(nomeCampo) && linha.toString().contains("null")) {
							...
							if (linha.toString().contains("!=")) {
								String textoIfStatement = condicional.getThenStmt().toString().replace("{", "").replace("}", "");
								System.out.println(textoIfStatement);
								
								Statement s = JavaParser.parseStatement(textoIfStatement);
