								...
								for (Node campo : camposCondicao) {
									for (Parameter parametro : parametrosMetodo) {
										...
										if (parametro.getNameAsString().toString().equals(campo.toString())
												...
												&& !txtCondicao.contains("equals")
												&& !txtCondicao.contains("++")	
												&& (txtCondicao.contains("=="))) {
											System.out.println("Achou parametro na condicional...");
											if (conditional != null && conditional.getThenStmt().toString().contains("new")
													 && conditional.getThenStmt().toString().contains("return")
													 && !conditional.getThenStmt().toString().contains("if")) {
												BlockStmt rowsIf = conditional.getThenStmt().asBlockStmt();
												for (Statement rowIf : rowsIf.getStatements()) {
													System.out.println("Linha IF: " + rowsIf.getStatements().get(0));
													if(rowsIf.getStatements().get(0).isReturnStmt()) {
														System.out.println("Linha tem return !!!");
														ReturnStmt returnStmt = (ReturnStmt) rowsIf.getStatements().get(0);
														System.out.println("Primeira parte do return: " + returnStmt.getChildNodes().get(0).getChildNodes());
														if(returnStmt.getChildNodes().get(0).getChildNodes().size() > 1
																&& rowsIf.getStatements().get(0).toString().contains("new")) {
															System.out.println("É retorno com parametros e não deve ser usado");
															return false;
														}
													}
												}
	...
	public void applyMethod(ProprietyClass object) {
			...
			for (Map.Entry<MethodDeclaration, List<Statement>> entrada : ondeModificar.entrySet()) {
				...
				for (Statement statement : condicionaisEncontrados) {
					...
					while(true) {
						System.out.println("Entro no laço do while factory: " + object.getAbsolutePath());
						...
						System.out.println("Nome classe factory: " + nomeClasse);
